...
public class AddPhongHocForm extends JFrame {
    ...
    private void addPhongHoc() throws Exception {
        ...
        // Kết nối đến cơ sở dữ liệu và thêm phòng học
        try (Connection conn = DBConnection.getConnection()) {
            String sql = "INSERT INTO PhongHoc (MaPhong, TenPhong, LoaiPhong, SucChua) VALUES (?, ?, ?, ?)";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setString(1, maPhong);
            stmt.setString(2, tenPhong);
            stmt.setString(3, loaiPhong);
            stmt.setInt(4, Integer.parseInt(sucChua)); // Chuyển đổi sức chứa sang kiểu int
            ...
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this, "Lỗi khi thêm phòng học: " + ex.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
        }

        // Đóng form và mở lại form danh sách phòng học
        dispose();
        new PhongHocCSVCForm();
    }
