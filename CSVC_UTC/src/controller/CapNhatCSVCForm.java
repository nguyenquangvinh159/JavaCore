...
public class CapNhatCSVCForm extends JFrame {
    ...
    public CapNhatCSVCForm(String ma, String ten, String loai, int soLuong, String tinhTrang, String donVi, DanhSachCSVCForm parentForm) {
        ...
        btnCapNhat.addActionListener(e -> {
        	String tenEdit = txtTen.getText().trim();
            String donViEdit = txtDonVi.getText().trim();
            String soLuongStr = txtSoLuong.getText().trim();
            if (tenEdit.isEmpty() || donViEdit.isEmpty() || soLuongStr.isEmpty()) {
            ...
            int soLuongEdit;
            try {
                soLuongEdit = Integer.parseInt(soLuongStr);
                if (soLuongEdit < 0) {
            ...
            try (Connection conn = DBConnection.getConnection()) {
                String sql = "UPDATE CoSoVatChat SET TenCSVC=?, LoaiCSVC=?, SoLuong=?, TinhTrang=?, DonViTinh=? WHERE MaCSVC=?";
                PreparedStatement stmt = conn.prepareStatement(sql);
                stmt.setString(1, tenEdit);
                stmt.setString(2, cbLoai.getSelectedItem().toString());
                stmt.setInt(3, soLuongEdit);
                stmt.setString(4, cbTinhTrang.getSelectedItem().toString());
                stmt.setString(5, donViEdit);
                stmt.setString(6, maCu);

                int rows = stmt.executeUpdate();
                if (rows > 0) {
                    JOptionPane.showMessageDialog(this, "Cập nhật thành công!");
                    if (parentForm != null) parentForm.loadData();
                    dispose();
                } else {
                    JOptionPane.showMessageDialog(this, "Không tìm thấy mã thiết bị để cập nhật.");
                }
            } catch (Exception ex) {
